/* eslint-disable */
window.PRSHOULDUSECONTINUATION = true;
(function () {
  ...
  var l = [
    p,
    'alignof,alignunion,asm,axiom,bool,concept,conceptmap,constcast,constexpr,decltype,dynamiccast,explicit,export,friend,inline,latecheck,mutable,namespace,nullptr,reinterpretcast,staticassert,staticcast,template,typeid,typename,using,virtual,where',
  ];
  var x = [
    p,
  ...
  function i(T) {
    ...
    S.push(
      ...
      [O, /^(?:[@_]?[A-Z]+[a-z][A-Za-z_$@0-9]*|\w+t\b)/, null],
  ...
  function b(ad) {
    ...
    function U() {
      var ag = window.PRSHOULDUSECONTINUATION ? W.now() + 250 : Infinity;
...
PR.registerLangHandler(
  PR.createSimpleLexer(
    ...
    [
      ...
      ['lang-css-kw', /^(-?(?:[a-z]|(?:\\[0-9a-f]+ ?))(?:[a-z0-9\-]|\\(?:\\[0-9a-f]+ ?))*)\s*:/i],
      [PR.PRCOMMENT, /^\/\*[^*]*\*+(?:[^\/*][^*]*\*+)*\//],
      [PR.PRCOMMENT, /^(?:<!--|-->)/],
      [PR.PRLITERAL, /^(?:\d+|\d*\.\d+)(?:%|[a-z]+)?/i],
      [PR.PRLITERAL, /^#(?:[0-9a-f]{3}){1,2}/i],
      [PR.PRPLAIN, /^-?(?:[a-z]|(?:\\[\da-f]+ ?))(?:[a-z\d\-]|\\(?:\\[\da-f]+ ?))*/i],
      [PR.PRPUNCTUATION, /^[^\s\w\'\"]+/],
    ]
  ),
  ['css']
);
PR.registerLangHandler(
  PR.createSimpleLexer(
    [],
    [[PR.PRKEYWORD, /^-?(?:[a-z]|(?:\\[\da-f]+ ?))(?:[a-z\d\-]|\\(?:\\[\da-f]+ ?))*/i]]
  ),
  ['css-kw']
);
PR.registerLangHandler(PR.createSimpleLexer([], [[PR.PRSTRING, /^[^\)\"\']+/]]), ['css-str']);
