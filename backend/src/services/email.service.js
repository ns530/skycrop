...
class EmailService {
  constructor() {
    this.provider = process.env.EMAIL_PROVIDER || 'console'; // 'sendgrid', 'ses', or 'console'
    this.fromEmail = process.env.EMAIL_FROM || 'noreply@skycrop.com';
    this.fromName = process.env.EMAIL_FROM_NAME || 'SkyCrop';
  ...
  _initSendGrid() {
    try {
      ...
      if (!apiKey) {
        logger.warn('SENDGRID_API_KEY not set, email service will use console logging');
      ...
      logger.warn('SendGrid not available, falling back to console logging', { error: error.message });
      this.provider = 'console';
    }
  }
  ...
  async sendEmail({ to, subject, html, text }) {
    ...
    try {
      if (this.provider === 'sendgrid') {
        ...
        return await this._sendViaSES({ to, subject, html, text });
      } else {
  ...
  async _sendViaSES({ to, subject, html, text }) {
  ...
  async sendHealthAlert(userEmail, fieldName, alertType, severity) {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>
          <a href="${process.env.FRONTEND_URL}/fields" style="
  ...
  async sendRecommendationEmail(userEmail, fieldName, recommendation) {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ` : ''}
        <p>
          <a href="${process.env.FRONTEND_URL}/fields/${recommendation.fieldId}/recommendations" style="
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #d4edda; padding: 15px; border-left: 4px solid #28a745; border-radius: 5px;">
          ...
          <p style="font-size: 24px; margin: 10px 0;">
            <strong>${prediction.predicted_yield_per_ha.toLocaleString()} kg/ha</strong>
          </p>
          <p style="font-size: 18px; margin: 10px 0;">
            Total: <strong>${prediction.predicted_total_yield.toLocaleString()} kg</strong>
          </p>
        </div>
        <h4>Confidence Interval:</h4>
        <p>
          Lower: ${prediction.confidence_interval.lower.toLocaleString()} kg/ha<br>
          Upper: ${prediction.confidence_interval.upper.toLocaleString()} kg/ha
        </p>
        <h4>Expected Revenue:</h4>
        <p style="font-size: 20px; color: #28a745;">
          <strong>LKR ${prediction.expected_revenue.toLocaleString()}</strong>
        </p>
        <p><strong>Estimated Harvest Date:</strong> ${prediction.harvest_date_estimate}</p>
        <p>
          <a href="${process.env.FRONTEND_URL}/fields/${prediction.field_id}/yield" style="
            background-color: #28a745;
            color: white;
            padding: 10px 20px;
            text-decoration: none;
