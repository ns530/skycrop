...
class APISmokeTester {
  ...
  async testAuthentication() {
    ...
    await this.runTest('User Registration', async () => {
      const response = await axios.post(`${APIBASE}/auth/register`, this.testUser, {
        timeout: 10000,
      });
    ...
    await this.runTest('Authentication Status', async () => {
      const response = await axios.get(`${APIBASE}/auth/status`, {
  ...
  async testFieldOperations() {
    ...
    await this.runTest('Field Creation', async () => {
      const response = await axios.post(`${APIBASE}/fields`, testFieldData, {
        headers: { Authorization: `Bearer ${this.authToken}` },
        timeout: 15000,
      });
    ...
    await this.runTest('Field Retrieval', async () => {
      const response = await axios.get(`${APIBASE}/fields/${this.testField.id}`, {
        headers: { Authorization: `Bearer ${this.authToken}` },
        timeout: 10000,
      });
    ...
    await this.runTest('Field Listing', async () => {
      const response = await axios.get(`${APIBASE}/fields`, {
        headers: { Authorization: `Bearer ${this.authToken}` },
        timeout: 10000,
      });
    ...
    await this.runTest('Field Update', async () => {
      ...
      const response = await axios.put(`${APIBASE}/fields/${this.testField.id}`, updateData, {
        headers: { Authorization: `Bearer ${this.authToken}` },
        timeout: 10000,
      });
  ...
  async testHealthData() {
    ...
    await this.runTest('Current Health Status', async () => {
      const response = await axios.get(`${APIBASE}/fields/${this.testField.id}/health/current`, {
        headers: { Authorization: `Bearer ${this.authToken}` },
        timeout: 15000,
      });
      ...
      return {
        status: response.status === 200 ? 'dataavailable' : 'nodata',
      };
    });
  }
  ...
  async testRecommendations() {
    ...
    await this.runTest('Recommendation Retrieval', async () => {
      const response = await axios.get(`${APIBASE}/fields/${this.testField.id}/recommendations`, {
        headers: { Authorization: `Bearer ${this.authToken}` },
        timeout: 15000,
      });
    ...
    try {
      await this.runTest('Recommendation Generation', async () => {
        const response = await axios.post(
          `${APIBASE}/fields/${this.testField.id}/recommendations/generate`,
          {},
          {
            headers: { Authorization: `Bearer ${this.authToken}` },
            timeout: 30000,
  ...
  async runTest(testName, testFunction) {
    ...
    console.log(
      `${testResult.status === 'passed' ? '✅' : '❌'} ${testName} (${testResult.duration}ms)`
    );
  }
  ...
  async cleanup() {
    try {
      ...
      if (this.testField && this.testField.id) {
        await sequelize.query('DELETE FROM fields WHERE id = ?', {
