...
describe('Controllers branch coverage (error and alt paths)', () => {
  beforeEach(() => {
    ...
    mockFieldSvc = {
      list: jest
        .fn()
        .mockResolvedValue({ items: [], total: 0, page: 1, pageSize: 20, cacheHit: false }),
    ...
    mockSatSvc = {
      ...
      queuePreprocess: jest.fn().mockResolvedValue({ jobid: 'job-1', status: 'queued' }),
      getJob: jest.fn().mockReturnValue({ jobid: 'job-1', status: 'completed' }),
    };

    mockMlSvc = {
      predict: jest.fn().mockResolvedValue({
        result: { success: true, data: { maskurl: 'http://mask' } },
        cacheHit: false,
    ...
    mockWeatherSvc = {
      getCurrentByField: jest
        .fn()
        .mockResolvedValue({ data: { ok: true }, meta: { cache: 'miss', source: 'provider' } }),
      getForecastByField: jest
        .fn()
        .mockResolvedValue({ data: { daily: [] }, meta: { cache: 'miss', source: 'provider' } }),
    };
  ...
  test('FieldController.list -> next(err) on service ValidationError', async () => {
    const req = { user: { user_id: 'user-1' }, query: {}, headers: {} };
    const res = makeRes();
    const next = jest.fn();
  ...
  test('FieldController.getById -> next(err) on NotFoundError', async () => {
    const req = { user: { user_id: 'user-1' }, params: { id: 'missing' }, headers: {} };
    const res = makeRes();
    const next = jest.fn();
  ...
  // ML Controller
  test('MLController.predict -> next(err) on AppError INVALIDINPUT', async () => {
    const req = { body: { return: 'maskurl' }, headers: { 'x-request-id': 'cid-1' } };
    const res = makeRes();
    const next = jest.fn();
    mockMlSvc.predict.mockRejectedValueOnce(new AppError('INVALIDINPUT', 'invalid', 400, {}));
  ...
  test('SatelliteController.getTile -> 304 Not Modified path', async () => {
    const req = {
      ...
      query: { date: '2025-10-10', bands: 'RGB', cloudlt: '20' },
  ...
  test('SatelliteController.getTile -> next(err) on ValidationError', async () => {
    const req = {
      params: { z: '-1', x: '0', y: '0' },
      query: { date: 'bad', bands: 'RGB', cloudlt: '20' },
      headers: {},
  ...
  test('WeatherController.forecast -> success 200 and payload', async () => {
    const req = { user: { user_id: 'user-1' }, query: { field_id: 'fid-1' }, headers: {} };
    const res = makeRes();
    const next = jest.fn();
