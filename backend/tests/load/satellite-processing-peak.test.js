...
// Custom metrics
const boundaryDetectionSuccessRate = new Rate('boundarydetectionsuccess');
const boundaryDetectionDuration = new Trend('boundarydetectionduration');

// Test configuration
export const options = {
  scenarios: {
    satelliteprocessingpeak: {
      ...
      tags: { testtype: 'satelliteprocessingpeak' },
    },
  },
  thresholds: {
    httpreqduration: ['p(95)<60000'], // 95% of requests should complete within 60s
    httpreqfailed: ['rate<0.1'], // Error rate should be below 10%
    boundarydetectionsuccess: ['rate>0.85'], // 85% success rate (matches AI accuracy)
    boundarydetectionduration: ['p(95)<60000'], // 95% under 60s
  },
};
...
export default function (data) {
  ...
  const boundaryResponse = http.post(`${BASEURL}/fields/detect-boundary`, payload, params);
