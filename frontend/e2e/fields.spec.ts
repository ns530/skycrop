...
test.describe('Fields CRUD Operations', () => {
  ...
  test('should navigate to create field page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /create field/i })).toBeVisible();
  });
  ...
  test('should view field details', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /field details/i })).toBeVisible();
  ...
  test('should sort fields by name', async ({ page }) => {
    ...
    const firstRowName = await page.getByRole('row').nth(1).getByRole('cell').first().textContent();
    const secondRowName = await page.getByRole('row').nth(2).getByRole('cell').first().textContent();
