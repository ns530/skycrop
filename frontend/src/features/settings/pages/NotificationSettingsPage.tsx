...
export const NotificationSettingsPage: React.FC = () => {
  ...
  const handleToggleEnabled = () => {
    if (!preferences.enabled && permission !== 'granted') {
      ...
      showToast({
        title: newPrefs.enabled ? 'Notifications enabled' : 'Notifications disabled',
  ...
  const handleToggleDoNotDisturb = () => {
    const newPrefs = { ...preferences, doNotDisturb: !preferences.doNotDisturb };
    setPreferences(newPrefs);
    notificationService.savePreferences(newPrefs);
    showToast({
      title: newPrefs.doNotDisturb ? 'Do Not Disturb enabled' : 'Do Not Disturb disabled',
  ...
  const handleToggleVibration = () => {
    const newPrefs = { ...preferences, vibrationEnabled: !preferences.vibrationEnabled };
    setPreferences(newPrefs);
    notificationService.savePreferences(newPrefs);
  };
  ...
  const notificationTypes: Array<{ type: NotificationType; label: string; description: string }> = [
    { type: 'health-alert', label: 'Health Alerts', description: 'Critical field health issues' },
    { type: 'weather-warning', label: 'Weather Warnings', description: 'Severe weather alerts' },
    { type: 'recommendation', label: 'AI Recommendations', description: 'New recommendations from AI' },
    { type: 'yield-update', label: 'Yield Updates', description: 'Yield prediction changes' },
    { type: 'system', label: 'System Messages', description: 'Important system updates' },
    { type: 'general', label: 'General News', description: 'General updates and news' },
  ];

  const isNotificationsSupported = notificationService.isSupported();

  return (
    <section aria-labelledby="notification-settings-heading" className="space-y-6">
      <header className="space-y-2">
        <h1 id="notification-settings-heading" className="text-2xl font-bold text-gray-900">
      ...
      <Card title="Do Not Disturb">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-900">Do Not Disturb mode</p>
              <p className="text-xs text-gray-600">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-600">Play sound with notifications</p>
            </div>
            <button
              type="button"
