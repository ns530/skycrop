...
export const FieldWeatherPage: React.FC = () => {
  ...
  const handleRetryForecast = () => {
    ...
    showToast({
      ...
      description: forecastError?.message ?? 'Attempting to reload weather forecast for this field.',
      variant: 'default',
    });
  };
    ...
    return (
      <section aria-labelledby="field-weather-heading" className="space-y-4">
        <header className="space-y-1">
          <h1 id="field-weather-heading" className="text-lg font-semibold text-gray-900">
            Field not found
          </h1>
    ...
    <section aria-labelledby="field-weather-heading" className="space-y-4">
      ...
      {showLocationMissingMessage && (
        <Card title="Weather unavailable">
          <p className="text-sm text-gray-700">
            Weather forecast is unavailable because this field does not have a valid location. Edit the field boundary to
            enable location-based forecasts.
          </p>
        </Card>
      )}
      ...
      {!showLocationMissingMessage && hasLocation && (
        <>
          ...
          {isForecastError && (
            <Card title="Unable to load weather forecast" status="poor" showStatusStripe>
              <div className="space-y-3">
                <p className="text-sm text-gray-700">
                  {forecastError?.message ?? 'Something went wrong while loading the weather forecast for this field.'}
                </p>
          ...
          {!isForecastLoading && !isForecastError && hasForecastDays && forecast && (
