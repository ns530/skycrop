...
export const WeatherOverviewPage: React.FC = () => {
  ...
  const handleRetryAlerts = () => {
    ...
    showToast({
      ...
      description: alertsError?.message ?? 'Attempting to reload active weather alerts.',
      variant: 'default',
    });
  };
  ...
  const handleRetryForecast = () => {
    ...
    showToast({
      ...
      description: forecastError?.message ?? 'Attempting to reload the regional 7-day forecast.',
      variant: 'default',
    });
  };
  ...
  return (
    <section aria-labelledby="weather-overview-heading" className="space-y-4">
      <header className="space-y-1">
        <h1 id="weather-overview-heading" className="text-lg font-semibold text-gray-900">
          Weather overview
        </h1>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-gray-500">
          <span>
            Regional centroid: {DEFAULT_REGION_LAT.toFixed(2)}, {DEFAULT_REGION_LON.toFixed(2)}
          </span>
      ...
      {isAlertsError && (
        <Card title="Unable to load weather alerts" status="poor" showStatusStripe>
          <div className="space-y-3">
            <p className="text-sm text-gray-700">
              {alertsError?.message ?? 'Something went wrong while loading active weather alerts.'}
            </p>
                  ...
                  {alerts?.map((alert) => {
                    ...
                    return (
                      <tr key={alert.id}>
                        ...
                        <td className="px-3 py-2 align-top text-gray-700">{startLabel}</td>
                        <td className="px-3 py-2 align-top text-gray-700">{endLabel}</td>
      ...
      {/* Regional forecast */}
      {!isForecastLoading && !isForecastError && hasForecastDays && regionalForecast && (
      ...
      {isForecastError && (
        <Card title="Unable to load regional forecast" status="poor" showStatusStripe>
          <div className="space-y-3">
            <p className="text-sm text-gray-700">
              {forecastError?.message ?? 'Something went wrong while loading the regional forecast.'}
            </p>
