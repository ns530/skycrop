 ...
 */
export const HealthIndexLegend: React.FC<HealthIndexLegendProps> = ({ indexType }) => {
  return (
    <Card title={`${indexType} legend`}>
      <div className="space-y-3">
        ...
        <ul className="space-y-2 text-xs text-gray-700" aria-label={`${indexType} color legend`}>
          {BASE_LEGEND.map((row) => (
            <li key={row.label} className="flex items-center justify-between gap-3">
              ...
              <span className="tabular-nums text-gray-500">{row.rangeHint}</span>
            </li>
          ))}
        </ul>
      </div>
