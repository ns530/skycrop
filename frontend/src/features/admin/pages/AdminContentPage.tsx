...
export const AdminContentPage: React.FC = () => {
  ...
  const { mutateAsync: upsertContent, isPending: isSaving } =
    useUpsertAdminContent();

  const [editorOpen, setEditorOpen] = useState(false);
  const [editor, setEditor] = useState<EditorState>(() => toEditorState());

  const [publishTarget, setPublishTarget] = useState<AdminContentItem | null>(
    null,
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast({
        ...
        description:
          apiError?.message ?? "Something went wrong while saving content.",
        variant: "error",
      });
    }
  };
  ...
  const handleRetry = async () => {
    try {
      ...
      showToast({
        ...
        description:
          apiError?.message ?? "Something went wrong while fetching content.",
        variant: "error",
      });
    }
  };
  ...
  const handleConfirmPublishToggle = async () => {
    ...
    try {
      ...
      showToast({
        ...
        description:
          apiError?.message ?? "Something went wrong while updating content.",
        variant: "error",
      });
    }
  };
    ...
    <section aria-labelledby="admin-content-heading" className="space-y-4">
      <header className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Manage news and articles shown to farmers.
          </p>
        </div>
      ...
      <Card>
        <div className="flex items-center justify-between gap-2 border-b border-gray-100 pb-3 mb-3 text-xs text-gray-600">
          <span>
            {isFetching ? "Refreshing content…" : `Total items: ${totalCount}`}
          </span>
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {!isLoading && !isError && sortedItems.length === 0 && (
                <tr>
                  <td
                    ...
                    className="px-4 py-6 text-center text-sm text-gray-500"
                  >
                    No content items yet. Use New content to create the first
                    announcement.
                  </td>
                </tr>
              )}
              ...
              {!isLoading &&
                ...
                sortedItems.map((item) => (
                  <tr
                    key={item.id}
                    className="hover:bg-gray-50 text-gray-900 align-top"
                  >
        ...
        {publishTarget && (
          <div className="space-y-3">
            <p className="text-sm text-gray-700">
              You are about to{" "}
              {publishTarget.status === "published" ? "unpublish" : "publish"}{" "}
              <span className="font-medium">{publishTarget.title}</span>.
            </p>
            <p className="text-xs text-gray-500">
              Published items may be visible to farmers in the mobile or web
              app, depending on configuration.
            </p>
            <div className="flex justify-end gap-2 pt-2">
              <Button
                size="sm"
                variant="secondary"
                onClick={closePublishToggle}
                disabled={isSaving}
              >
                Cancel
              </Button>
