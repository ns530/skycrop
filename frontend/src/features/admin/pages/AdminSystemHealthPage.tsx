      ...
      <header className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-600">
            Real-time view of API, ML, and satellite ingest status. Use this to
            understand how SkyCrop services are behaving.
          </p>
        </div>
        <Button
          size="sm"
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            >
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={mapServiceToBadgeClasses(status.api)}>
                    {mapServiceToLabel(status.api)}
                  </span>
                </div>
                <p className="text-sm text-gray-700">
                  The API provides all core platform capabilities including
                  authentication, fields, health, and recommendations.
                </p>
                {status.api === "degraded" && (
                  <p className="text-xs text-status-fair">
                    API is degraded. Expect slower responses and occasional
                    timeouts for farmers.
                  </p>
                )}
                {status.api === "down" && (
                  <p className="text-xs text-status-poor">
                    API is down. Farmers may be unable to sign in or view
            ...
            >
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-600">Status</span>
                  <span className={mapServiceToBadgeClasses(status.mlService)}>
                    {mapServiceToLabel(status.mlService)}
                  </span>
                </div>
                <p className="text-sm text-gray-700">
                  ML services power health indices, yield predictions, and
                  recommendation models for farmer fields.
                </p>
                {status.mlService === "degraded" && (
                  <p className="text-xs text-status-fair">
                    ML service is degraded. Health maps and recommendations may
                    take longer to generate.
                  </p>
                )}
                {status.mlService === "down" && (
                  <p className="text-xs text-status-poor">
                    ML service is down. New health insights or recommendations
                    may not be available and existing ones may be stale.
                  </p>
                )}
              </div>
            </Card>
            ...
            >
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-600">Status</span>
                  <span
                    className={mapServiceToBadgeClasses(status.satelliteIngest)}
                  >
                    {mapServiceToLabel(status.satelliteIngest)}
                  </span>
                </div>
                <p className="text-sm text-gray-700">
                  Satellite ingest handles fetching, processing, and storing new
                  imagery used for field monitoring.
                </p>
                {status.satelliteIngest === "degraded" && (
                  <p className="text-xs text-status-fair">
                    Ingest is degraded. Expect delays before new satellite
                    scenes appear in field views.
                  </p>
                )}
                {status.satelliteIngest === "down" && (
                  <p className="text-xs text-status-poor">
                    Ingest is down. No new imagery will be processed until this
                    is resolved; health data may become outdated.
                  </p>
                )}
              </div>
            </Card>
