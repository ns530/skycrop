...
export const FieldsListPage: React.FC = () => {
  ...
  const canGoPrevious = useMemo(() => (pagination ? pagination.page > 1 : page > 1), [pagination, page]);
  ...
  const handleRetry = async () => {
    try {
      ...
      showToast({
        ...
        description: apiError?.message ?? 'Something went wrong while fetching fields.',
        variant: 'error',
      });
    }
  };
  ...
  const renderRow = (field: FieldSummary) => {
    ...
    return (
      ...
      >
        ...
        <td className="px-4 py-3 text-xs text-gray-700">{field.areaHa.toFixed(2)} ha</td>
        <td className="px-4 py-3 text-xs text-gray-500">
          <span className="block">Created: {new Date(field.createdAt).toLocaleDateString()}</span>
          <span className="block">Updated: {new Date(field.updatedAt).toLocaleDateString()}</span>
        </td>
  ...
  return (
    <section aria-labelledby="fields-list-heading" className="space-y-4">
      <header className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 id="fields-list-heading" className="text-lg font-semibold text-gray-900">
            Your fields
          </h1>
      ...
      <Card>
          ...
          <table className="min-w-full divide-y divide-gray-100 text-left text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-4 py-2 text-xs font-semibold uppercase tracking-wide text-gray-500 text-right">
                  Actions
                </th>
              </tr>
            </thead>
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center">
                    <ErrorState
                      ...
                      message={error?.message ?? 'Something went wrong while fetching fields.'}
                      onRetry={handleRetry}
                    />
                  </td>
                </tr>
              ...
              {!isLoading && !isError && fields.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center text-sm text-gray-500">
                    You haven't added any fields yet. Use the "Add field" button to register your first
                    field.
                  </td>
                </tr>
              )}
        ...
        <footer className="mt-4 flex items-center justify-between gap-3 border-t border-gray-100 pt-3 text-xs text-gray-600">
          <div>
            {pagination ? (
              <span>
                Page {pagination.page} · Showing {fields.length} of {pagination.total} fields
              </span>
            ) : (
