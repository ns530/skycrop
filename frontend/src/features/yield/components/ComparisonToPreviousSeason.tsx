...
export const ComparisonToPreviousSeason: React.FC<ComparisonToPreviousSeasonProps> = ({
  currentYield,
  previousYield,
}) => {
    ...
    <div className="bg-blue-50 rounded-lg p-3">
      <div className="flex items-center justify-between text-sm">
        ...
        <span className={`font-medium ${isIncrease ? 'text-green-600' : 'text-red-600'}`}>
          {isIncrease ? '+' : ''}{formatNumber(percentageChange)}%
        </span>
      </div>
      <div className="mt-2 text-xs text-gray-500">
        Current: {formatNumber(currentYield)} kg/ha • Previous: {formatNumber(previousYield)} kg/ha
      </div>
    </div>
  );
};
