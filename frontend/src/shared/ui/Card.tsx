  ...
  return (
    ...
    >
      {showStatusStripe && status !== 'default' && (
        <div className={clsx('w-1 rounded-l-lg', statusColor)} aria-hidden="true" />
      )}
      <div className={clsx('flex-1 p-4 sm:p-5', showStatusStripe && status !== 'default' && 'ml-0')}>
        {title && (
          <h2 className="text-sm font-semibold text-gray-900 mb-2" aria-label={title}>
            {title}
          </h2>
        )}
