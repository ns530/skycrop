 ...
 */
export const generateFieldHealthReportPDF = (
  data: FieldHealthReportData,
): void => {
  const doc = new jsPDF();
 ...
 */
export const generateYieldForecastReportPDF = (
  data: YieldForecastReportData,
): void => {
  const doc = new jsPDF();
  ...
  if (data.predictions.length > 0) {
    ...
    doc.setFontSize(10);
    doc.text(
      `Latest Prediction: ${latestPred.predicted_yield_per_ha.toFixed(0)} kg/ha`,
      14,
      finalY + 18,
    );
    doc.text(
      `Total Expected Yield: ${latestPred.predicted_total_yield.toFixed(0)} kg`,
      14,
      finalY + 24,
    );
    doc.text(
      `Expected Revenue: LKR ${latestPred.expected_revenue.toLocaleString()}`,
      14,
      finalY + 30,
    );
    doc.text(
      `Estimated Harvest: ${new Date(latestPred.harvest_date_estimate).toLocaleDateString()}`,
      14,
      finalY + 36,
    );
  }
